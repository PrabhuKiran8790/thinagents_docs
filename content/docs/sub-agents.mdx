...
- **Parallelism:** Enable parallel tool calls and sub-agent execution for faster responses.
...
w_agent = Agent(
    ...
    tools=[fetch_weather],
    instructions=["Summarise the weather in a funny way"],
)
...
The `thinagents` library also provides utilities to visualize agent and sub-agent relationships, making it easier to debug and design complex agent systems.
